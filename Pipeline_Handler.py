...
class PreprocessingTransformer(BaseEstimator, TransformerMixin):
    def transform(self, X):
        ...
        # LTV and FICO binning
        X = X[X['CreditScore'] != 0]
        fico_bins = [300, 580, 670, 740, 800, 851]
        fico_labels = ['Very Poor', 'Poor', 'Fair', 'Good', 'Excellent']
        X['FICO_Category'] = pd.cut(X['CreditScore'], bins=fico_bins, labels=fico_labels, right=False)

        bins = [0, 80, 90, 100]
        labels = ['Low LTV', 'Medium LTV', 'High LTV']
        X['LTV_Group'] = pd.cut(X['LTV'], bins=bins, labels=labels, right=False)

        # YearsInRepayment binning
        X['YearsInRepayment'] = X['MonthsInRepayment'] / 12
        bins = [0, 4, 8, 12, 16, 20, float('inf')]
        labels = ['0-4 years', '4-8 years', '8-12 years', '12-16 years', '16-20 years', '20+ years']
        X['YearsInRepayment_Group'] = pd.cut(X['YearsInRepayment'], bins=bins, labels=labels, right=False, include_lowest=True)

        # Encode categorical features
        X['LTV_Group_Encoded'] = X['LTV_Group'].map({'Low LTV': 0, 'Medium LTV': 1, 'High LTV': 2})
        X['FICO_Category_Encoded'] = X['FICO_Category'].map({'Very Poor': 0, 'Poor': 1, 'Fair': 2, 'Good': 3, 'Excellent': 4})
        X['YearsInRepayment_Group_Encoded'] = X['YearsInRepayment_Group'].map({'0-4 years': 0, '4-8 years': 1, '8-12 years': 2, '12-16 years': 3, '16-20 years': 4, '20+ years': 5})
        ...
        X = X.drop(columns=['CreditScore', 'LTV', 'FirstTimeHomebuyer', 'YearsInRepayment',
                             'Channel','LTV_Group', 'FICO_Category',
                            'YearsInRepayment_Group','DTIBin'])
...
class PipelineHandler:
    def __init__(self, clf, reg):
        # Initialize the the classifecation (GBM) and regression model (lasso)
        self.clf = clf
        self.reg = reg
    ...
    def fit(self, X, y_class):
        ...
        classification_features = ['MonthsDelinquent', 'FICO_Category_Encoded',
                               'PaymentProgressRatio', 'MonthsInRepayment',
                               'YearsInRepayment_Group_Encoded', 'NumBorrowers',
                               'Channel_Frequency', 'LTV_Group_Encoded',
                               'isHomeFirstBuyer', 'OCLTV', 'DTI', 'OrigInterestRate']
        ...
        self.clf.fit(X_train_scaled[classification_features], y_class_train)  

        # Predict delinquency status
        y_class_pred = self.clf.predict(X_test_scaled[classification_features]) 
        ...
        # Filter delinquent borrowers by using y_class_train to filter y_reg_train and y_class_test to filter y_reg_test

        X_filtred_reg_train = X_train_scaled[y_class_train == 0]
        y_reg_filtered_train = y_reg_train[y_class_train == 0]  

        X_filtred_reg_test = X_test_scaled[y_class_test == 0]
        y_reg_filtered_test = y_reg_test[y_class_test == 0]   
        ...
        b = mean_squared_error(y_reg_filtered_test, y_reg_pred, squared=False)  
    ...
    def predict(self, X):
        ...
        #  regression features
        regression_features = ['OrigUPB', 'MonthlyPayment', 'total_amount',
                               'interest_amount', 'MonthlyIncome', 'CurrentPrincipal', 'DTIBins']
